...
function delay2(f, ms) { // f f(x){} , ms: 1000

    // `this` ve diğer argümanların setTimeout içerisindeki saklayıcıdan iletilmesini sağlar.
    return function (...args) {

        let savedThis = this; // this: Timeout

        setTimeout(function () {

            f.apply(savedThis, args); // args ın içinde (1) ['test'] var
...
f1000("test"); // "test" yazısı 1000ms sonra gelir. , f1000 değişkeninin içine f fonksiyonunu atıyor
f1500("test"); // "test" yazısı 1500ms sonra gelir.