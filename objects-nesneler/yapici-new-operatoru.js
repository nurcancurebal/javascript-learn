...
{ ... } yazımı bir objenin yaratılmasına yarar.Fakat bir objenin benzeri farklı objeler yaratmak istenebilir.Örneğin farklı
kullanıcılar, farklı menü değerleri.
...
console.log(kullanici); // Kullanici { isim: 'Nurcan', yoneticiMi: false }
...
/* Eğer başka bir kullanici oluşturmak istiyorsanız, yapmanız gereken new Kullanici("Macide"), new Kullanici("Muzaffer")
gibi kullanmaktır.Doğrudan her defasında obje tanımlamaktan daha kısa ve anlaşılması kolaydır.
Yapıcı fonksiyonların amacı – tekrar kullanılabilecek objeleri yaratan kodun uygulanmasıdır.
Dikkat edecek olursanız, herhangi bir fonksiyon yapıcı fonksiyon olabilir.Her fonksiyon new ile çalıştırılabilir, ve
yukarda anlatılan algoritmaya göre çalışır. “Yapıcı fonksiyon isimleri büyük harfle başlamalıdır” aslında genel bir
ittifaktır, bunu daha da açıklayıcı yapmak için bu fonksiyonlar new ile çağırılmalıdır. */

new function () {
    //…
}

/* Eğer birçok satırdan oluşan kodda amacınız sadece karmaşık bir obje yapmak ise, bunları yapıcı fonksiyon içine aşağıdaki
gibi almak mümkündür: */
...
/* Yapıcı fonksiyon tekrar çağırılamaz çünkü hiçbir yere kayıt edilmemiştir, sadece yaratılır ve çağırılır.Böylece yapıcı
metod ilerde tekrar kullanılmayacağına garanti verir. */
...
/* Fonksiyon içinde, bu fonksiyon new ile mi yoksa new olmadan mı çağırılmış bu new.target özelliği kullanılarak
anlaşılabilir.
...
// new kullanılmadan:
Kullanici4(); // undefined, new.target undefined döndürür.

// new kullanılarak:
new Kullanici4(); // [Function: Kullanici4], gibi ekrana fonksiyonu yazar

// new ve normal sözdiziminin(syntax) aynı çalışmasını sağlar:

function Kullanici5(isim) {

    if (!new.target) { // eğer şart yerine getirilmezse
...
let figen = Kullanici5("Figen"); // çağrıyı new Kullanici5(isim) fonksiyonuna yönlendirir.

console.log(figen.isim); // Figen

/* Bu yaklaşım bazı kütüphanelerde yazımı daha esnek yapabilmek amacıyla kullanılır.Her yerde kullanılması o kadar da iyi
değildir.Çünkü new ne olup bittiği hakkında bilgi vermektedir.new ile yeni bir obje yaratıldığını anlayabiliyorsunuz ki bu
da iyi bir şeydir. */


/* -> Yapıcı metodun return sözcüğü
Genelde yapıcı metodların return sözcüğü yoktur.Amaçları gerekli olan bilgileri this içine yazmaktır ve bu da otomatik
olarak sonuçtur.
...
Yapıcı fonksiyon kullanmak objeye mükemmel esneklik sağlar.Yapıcı fonksiyon parametreleri tanımlar ve objenin nasıl
yapılacağını, ne konulması gerektiğini belirtir.
Tabiki this'e sadece özelliklerde değil metodlar içerisinde de ekleme yapılabilir.
Örneğin, aşağıdaki new Kullanici8(isim) verilen isim ile yeni bir obje oluşturur.Bu obje selamVer metoduna sahiptir. */

function Kullanici8(isim) {
...
let timur = new Kullanici8("Timur");

timur.selamVer(); // Benim adım: Timur

console.log(timur); // Kullanici8 { isim: 'Timur', selamVer: [Function (anonymous)] }

console.log(Kullanici8()); // undefined, direk ulaşılamıyor fonksiyona