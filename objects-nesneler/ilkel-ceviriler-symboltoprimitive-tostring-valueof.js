...
Tip Dönüşümleri bölümünde sayısal, karakter ve boolean çevrimleri gösterildi. Fakat objeler için daha sonra değilineceği
söylendi. Şimdi objeler ve semboller hakkında bilginiz olduğuna göre bunu anlaması daha da kolay olacaktır.
Objeler için boolean çevirimi bulunmamaktadır çünkü tüm objeler boolean dahilinde true'dur. Bundan dolayı sadece sayısal ve
karakter çevrimleri mevcuttur.
Sayısal çevirim obje çıkarıldığında veya metematiksel fonksiyonlar uygulandığında meydana gelir. Örneğin Date objesi (Tarih
ve Zaman bölümünde anlatılacak) çıkarılabilir ve date1-date2 bu iki tarih arasındaki zaman farkını verir.
...
Objenin ilkel bir tip olarak gerekli olduğu durumlarda, örneğin console.log veya matematiksel uygulamalarda, ToPrimitive
algoritması kullanılarak bu işlem yapılır. (Özellikleri)
...
Örneğin, binary + karakter dizisi için birleştirme işlemi yaparken sayı için toplama işlemi yapar, bundan dolayı hem
karakter hem sayı olabilir. Veya objeyi karakter, sayı veya sembol ile == şeklinde karşılaştırırken */
...
/* Büyüktür/küçüktür operatörü <> karakter ve sayı ile çalışabilir. Fakat yine de default değil de sayı ipucunu kullanır.
Bu tarihsel nedenlerden ötürü böyledir.
Pratikte tüm objeler ( Date objesi hariç ) "default" çevrimini “number” çevrimi ile aynı şekilde yaparlar. */

/* Sadece üç çeşit ipucu(hint) bulunmaktadır. boolean bulunmamaktadır, çünkü her obje zaten boolean true döndürür.
"default" ve "number" için de aynı olduklarını varsayarsanız, sadece iki çevrim bulunmaktadır.
...
/* Koddan da gördüğünüz üzere kullanici çeviriye göre karakter veya para olabiliyor. Tek bir metod
kullanici[Symbol.toPrimitive] tüm çeviri durumlarını karşılıyor. */


/* -> toString/valueOf
toString ve valueOf gibi metodlar ilk JavaScript çıktığı zamandan kalma metodlardır. Sembol değillerdir( O zamanlar sembol
yoktu), yani normal metodlardır. Eski tip çevirim düzenlemesi yapabilmenize olanak verir.
...
/* -> Dönüş Tipleri
İlkel tip çevrilerinde bilinmesi gereken en önemli olay bu metodların illa da ipucunda alınan tiplere dönüştürmesine gerek
yoktur.
toString() illa karakter döndürecek diye veya Symbol.toPrimitive içerisinde eğer ipucu sayı ise sayı döndürecek diye bir
kural yoktur.
...
/* -> Tarihi Nedenlar
toString veya valueOf ilkel tip döndürmelidir. Eğer obje dönderiyorsa bir hata vardır. Fakat bu obje sadece pas geçilir
aynı metodu olmadığında sadece pas geçildiği gibi.
